...
{
    public class MQTTApplicationV3 : MQTTApplication
    {
        public override string Name => "MQTT V3.x Protocol";
        ...
        protected virtual bool OnContent(string clientid, Connect e, HttpRequest websocket, ISession tcp)
        {
            ...
            {
                ack.Status =  ConnectStatus.NotSupport;
            ...
            if (SessionVerify)
            {
                using (MQTTDB context = new MQTTDB())
                {
                    ...
                    if (item == null || e.Password != item.PWD || !item.Enabled)
                    {
                        ...
                        ack.Status =  ConnectStatus.BadNameOrPassword;
                        Reply(ack, websocket, tcp);
            ...
            ack.Status = ConnectStatus.Success;
            Reply(ack, websocket, tcp);
            if (websocket == null)
            {
        ...
        protected virtual void OnUnSubscribe(MQTTUser client, UnSubscribe e)
        {
            ...
            ack.Identifier = e.Identifier;
            client.Send(ack);
            UnRegisterSubscribe(e.Subscription, client);
